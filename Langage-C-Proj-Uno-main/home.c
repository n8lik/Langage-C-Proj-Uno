...
int home_page()
{
    ...
    SDL_BlitSurface(parametre, NULL, screen, &parametrepos);
     ...
     SDL_Surface *shopImage = IMG_Load("assets/shop.jpg"); // Renommé pour éviter la confusion
    if (!shopImage) {
        fprintf(stderr, "Unable to load image: %s\n", IMG_GetError());
        TTF_CloseFont(font);
        SDL_FreeSurface(backgroundImage);
    ...
    // Position de l'image sur l'écran, renommé en shopImageRect pour clarifier
    SDL_Rect shopImageRect = {x_position6, y_position6, 0, 0};
    SDL_BlitSurface(shopImage, NULL, screen, &shopImageRect);
        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (x >= x_position6 && x <= x_position6 + shopImage->w && y >= y_position6 && y <= y_position6 + shopImage->h) {
                            // Corrigé pour appeler la fonction shop() correctement
                            shop(screen);
