...
card *create_deck() {
    ...
    for (Color c = YELLOW; c <= GREEN; c++) {
        for (int i = 10; i <= 12; i++) { // +2, sens, passe ton tour
            for (int j = 0; j < 2; j++) { // 2 de chaque
                deck[index].value = i;
                deck[index].color = c;
                deck[index].type = SPECIAL;
                index++;
            }
        }
    }
    ...
    for (int i = 13; i <= 14; i++) { // joker et +4
        for (int j = 0; j < 4; j++) { // 4 de chaque
            deck[index].value = i;
